...
import { Circunscripcion, PersonalCitrep } from '@/types/circunscripcion';
import { fetchPersonalByCitrep } from '@/services/api';

const ITEMS_PER_PAGE = 5;

interface InfoPanelCircProps {
  circunscripcion: Circunscripcion | null;
}

// Componente Modal para el detalle del agente (usando Portal)
function AgenteModal({ agente, onClose }: { agente: PersonalCitrep; onClose: () => void }) {
    ...
    >
      ...
      >
        ...
        <div className="space-y-3">
          <DetailRow icon="id" label="ID" value={agente.id_personal_citrep?.toString()} />
          <DetailRow icon="number" label="Nro" value={agente.nro?.toString()} />
          <DetailRow icon="location" label="Citrep" value={agente.citrep} />
          <DetailRow icon="location" label="Departamento" value={agente.departamento} />
          <DetailRow icon="location" label="Municipio" value={agente.municipio_principal} />
          ...
          <DetailRow icon="role" label="Estado" value={agente.estado} />
        </div>
...
export default function InfoPanelCirc({ circunscripcion }: InfoPanelCircProps) {
  const [agentes, setAgentes] = useState<PersonalCitrep[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [selectedAgente, setSelectedAgente] = useState<PersonalCitrep | null>(null);

  useEffect(() => {
    const loadAgentes = async () => {
      if (!circunscripcion?.citrep) {
        setAgentes([]);
        setCurrentPage(1);
        return;
      }
      ...
      setIsLoading(true);
      // Usar el citrep para buscar los agentes
      const data = await fetchPersonalByCitrep(circunscripcion.citrep);
      setAgentes(data);
      setCurrentPage(1);
      setIsLoading(false);
    };
    ...
    return (
      <div className="bg-white rounded-xl shadow-lg p-4 w-[300px]">
        ...
        <p className="text-sm text-gray-500 italic">
          Selecciona una circunscripción para ver los agentes asignados.
        </p>
      </div>
    );
  }
