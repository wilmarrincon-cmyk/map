...
import { fetchAllPersonalCitrep, fetchPersonalByCitrep } from '@/services/api';
import { Circunscripcion, PersonalCitrep } from '@/types/circunscripcion';

interface CargoStatsCircProps {
...
export default function CargoStatsCirc({ selectedCircunscripcion }: CargoStatsCircProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      let personal: PersonalCitrep[];
      
      if (selectedCircunscripcion?.citrep) {
        personal = await fetchPersonalByCitrep(selectedCircunscripcion.citrep);
      } else {
        personal = await fetchAllPersonalCitrep();
      }
